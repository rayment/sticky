...
/*
 * tcp_single_noblock.c
 * Single non-blocking client/server TCP net test suite.
 *
...
func_srv(void *data)
{
	...
	/* bind to port */
	TEST(
		socket = S_tcp_bind(srv_data->family, PORT, S_FALSE);
	, socket
	, "srv: S_tcp_bind");
